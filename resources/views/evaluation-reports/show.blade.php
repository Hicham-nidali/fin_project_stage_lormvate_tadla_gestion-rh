...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>{{ $report->title }}</h1>
        <div>
            <a href="{{ route('evaluation-reports.index') }}" class="btn btn-secondary me-2">
                <i class="fas fa-arrow-left"></i> Retour
            </a>
            @if($report->status == 'draft' && $report->created_by == Auth::user()->id)
                <a href="{{ route('evaluation-reports.edit', $report->id) }}" class="btn btn-primary me-2">
                    <i class="fas fa-edit"></i> Modifier
                </a>
                <form method="POST" action="{{ route('evaluation-reports.send', $report->id) }}" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-success" onclick="return confirm('Envoyer ce rapport à l\'administration RH ?')">
                        <i class="fas fa-paper-plane"></i> Envoyer à RH
                    </button>
                </form>
            @endif
    ...
    <div class="card mb-4">
            ...
            <div class="row">
                ...
                <div class="col-md-4 text-end">
                    ...
                    @elseif($report->status == 'sent')
                        <span class="badge bg-primary fs-6">Envoyé</span>
                    @else
        ...
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <strong>Examiné le:</strong> {{ $report->reviewed_at->format('d/m/Y H:i') }}<br>
                    <strong>Examiné par:</strong> {{ $report->reviewer->name }}
                </div>
                @endif
            </div>
            @endif
    ...
    <div class="card mb-4">
        <div class="card-header bg-light">
            <h5 class="mb-0">Performance Globale des Employés</h5>
        </div>
        <div class="card-body">
